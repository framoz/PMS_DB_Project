...
def get_random_id(table_name, id_column):
    """Fetch a random ID from a given table."""
...
def populate_product_sale(n=10):
    ...
    for _ in range(n):
        ...
        if id_product and id_sale:
            try:
                ...
                print(f"Failed to insert ProductSale: {e}")
            else:
                print(f"Inserted ProductSale: Product={id_product}, Sale={id_sale}, Quantity={quantity}")

def populate_invoice_sale(n=5):
    ...
    for _ in range(n):
        ...
        if id_invoice and id_sale:
            try:
                ...
                print(f"Failed to insert InvoiceSale: {e}")
            else:
                print(f"Inserted InvoiceSale: Invoice={id_invoice}, Sale={id_sale}")

# Populate the tables
   # Insert 5 random invoice-sale connections
...
def populate_tables():
    ...
    populate_sale()
    populate_contact()
    populate_product_sale()  # Insert 10 random product-sale connections
    populate_invoice_sale()  # Added to populate Contact table
    conn.commit()
